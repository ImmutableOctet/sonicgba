...
Class GameState Extends State
	...
	Public
		...
		Public Method logic:Void()
			...
			Select (Self.state)
				Case 0
					...
					If (Key.buttonPress(PLANE_VELOCITY)) Then
					EndIf
					...
					If (StageManager.isStageTimeover()) Then
						...
						Else
							Self.overtitleID = Def.TOUCH_HELP_HEIGHT
					...
					ElseIf (Self.overtitleID = Def.TOUCH_HELP_HEIGHT) Then
						Self.movingTitleSpeedX = 8
					EndIf
					
				Case 1
					gamePauseLogic()
				Case PAUSE_RACE_OPTION
					stagePassLogic()
				...
				Case PAUSE_RACE_INSTRUCTION
					...
					If (loadingEnd()) Then
						...
						If (Not (StageManager.getStageID() = LOADING_TIME_LIMIT And PlayerObject.isDeadLineEffect)) Then
						...
						If (StageManager.isNextGameStageDirectedly) Then
							Self.state = 36
						...
						PlayerObject.clipMoveInit(0, (SCREEN_HEIGHT Shr 1) - LOADING_TIME_LIMIT, 20, SCREEN_WIDTH, 60)
						...
						If (IsGameOver) Then
							...
							Else
								...
								ElseIf (PlayerObject.stageModeState = 1) Then
									setStateWithFade(PAUSE_RACE_TO_TITLE)
								EndIf
				...
				Case PlayerTails.TAILS_ANI_CELEBRATE_2
					...
					ElseIf (Self.overtitleID = Def.TOUCH_HELP_HEIGHT And Self.overcnt = 28) Then
						Self.state = 11
					EndIf
		...
		Public Method draw:Void(g:MFGraphics)
			...
			Select (Self.state)
				...
				Default
					...
					If (Self.state = 15 Or Self.state = 27 Or Self.state = 16 Or Self.state = 17) Then
						...
						g.setClip(Self.display[1][0], Self.display[1][1] - LOADING_TIME_LIMIT, SCREEN_WIDTH, 20)
					...
					If (Self.state = 36 Or Self.state = 37) Then
						g.setColor(0)
						MyAPI.fillRect(g, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)
						
						If (loadingType = PLANE_VELOCITY) Then
							g.setColor(MapManager.END_COLOR)
							MyAPI.fillRect(g, 0, ((SCREEN_HEIGHT Shr 1) - 36) - LOADING_TIME_LIMIT, SCREEN_WIDTH, 20)
						EndIf
					EndIf
					...
					If (Self.state = 40) Then
						State.drawFade(g)
					EndIf
					
					If (Self.state = PAUSE_RACE_OPTION) Then
						PlayerObject.stagePassDraw(g)
					EndIf
			...
			If (isDrawTouchPad) Then
				If (Not (Self.state = PAUSE_RACE_INSTRUCTION Or Self.state = BIRD_SPACE_2)) Then
					drawTouchKeyDirect(g)
				EndIf
				
				If (Self.state = BIRD_SPACE_2 And Self.endingState > PAUSE_RACE_TO_TITLE) Then
					drawTouchKeyDirect(g)
				EndIf
			EndIf
	...
	Private
		...
		Private Method drawWhiteBar:Void(g:MFGraphics, x:Int, y:Int)
			g.setColor(MapManager.END_COLOR)
			MyAPI.fillRect(g, x, y - LOADING_TIME_LIMIT, SCREEN_WIDTH, 20)
		End
		...
		Private Method drawHugeStageName:Void(g:MFGraphics, stageid:Int, x:Int, y:Int)
			...
			For (Int x1 = Self.selectMenuOffsetX - 294; x1 < SCREEN_WIDTH * PLANE_VELOCITY; x1 += SpecialMap.MAP_HEIGHT)
				MFGraphics mFGraphics = g
				stageInfoAniDrawer.draw(mFGraphics, stage_id + PAUSE_RACE_INSTRUCTION, x1, (y - LOADING_TIME_LIMIT) + PLANE_VELOCITY, False, 0)
			Next
		End
		...
		Public Method pause:Void()
			...
			If (Self.state <> STAFF_SHOW_TIME And Self.state <> 18 And Self.state <> 1 And Self.state <> 20 And Self.state <> 23 And Self.state <> 21) Then
				If (Self.state = 0 And GameObject.player.isDead) Then
					Self.interrupt_state = Self.state
					Self.state = 18
					interruptInit()
				ElseIf (Self.state = PAUSE_RACE_INSTRUCTION Or Self.state = 35 Or Self.state = BIRD_SPACE_2 Or Self.state = PAUSE_RACE_OPTION Or Self.state = 7 Or Self.state = 8 Or Self.state = VISIBLE_OPTION_ITEMS_NUM Or Self.state = 29 Or Self.state = LOADING_TIME_LIMIT Or Self.state = PAUSE_OPTION_ITEMS_NUM Or Self.state = 29 Or Self.state = 41 Or Self.state = 42 Or Self.state = 28 Or Self.state = 11 Or Self.state = 12 Or Self.state = 13 Or Self.state = 30 Or Self.state = 31 Or Self.state = 32 Or Self.state = 33 Or Self.state = 34 Or Self.state = 38 Or Self.state = 39 Or Self.state = 43 Or Self.state = 44 Or Self.state = STAFF_SHOW_TIME) Then
					Self.interrupt_state = Self.state
					Self.state = 18
					interruptInit()
					Print("interrupt")
		...
		Public Method BP_shopLogic:Void()
			...
			ElseIf (Key.press(PLANE_VELOCITY)) Then
				Self.state = 1
				GameObject.IsGamePause = True
		...
		Private Method initStageIntroType1Conf:Void()
			...
			Self.display[PAUSE_RACE_INSTRUCTION][1] = (SCREEN_HEIGHT Shr 1) - LOADING_TIME_LIMIT
			Self.display[PAUSE_RACE_INSTRUCTION][PLANE_VELOCITY] = 0
