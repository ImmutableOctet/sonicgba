...
Class PlayerObject Extends MoveObject Implements Focusable, ACWorldCalUser Abstract
	...
	static {
		...
		iArr[TERMINAL_NO_MOVE] = TERMINAL_NO_MOVE
		iArr[TERMINAL_RUN_TO_RIGHT_2] = TERMINAL_RUN_TO_RIGHT_2
		iArr[TERMINAL_SUPER_SONIC] = TERMINAL_SUPER_SONIC
		CHARACTER_LIST = iArr
		characterID = WALK_COLLISION_CHECK_OFFSET_Y
		EFFECT_LOOP = New Bool[]{CAN_BE_SQUEEZE, CAN_BE_SQUEEZE}
		MOVE_POWER = ANI_PULL_BAR_MOVE
		MOVE_POWER_IN_AIR = 92
		MAX_VELOCITY = 1280
		MOVE_POWER_REVERSE = 336
		MOVE_POWER_REVERSE_BALL = 96
		FAKE_GRAVITY_ON_WALK = NUM_DISTANCE_BIG
		FAKE_GRAVITY_ON_BALL = SpecialMap.MAP_HEIGHT
		JUMP_START_VELOCITY = -1208 - GRAVITY
		JUMP_INWATER_START_VELOCITY = -1304 - GRAVITY
		JUMP_PROTECT = (-GRAVITY) - GRAVITY
		SPIN_START_SPEED_1 = 1440
		SPIN_START_SPEED_2 = 2400
		SPIN_INWATER_START_SPEED_1 = 2160
		SPIN_INWATER_START_SPEED_2 = 3600
		HURT_POWER_X = PlayerSonic.BACK_JUMP_SPEED_X
		HURT_POWER_Y = -992
		JUMP_RUSH_SPEED_PLUS = 480
		JUMP_REVERSE_POWER = LOOK_COUNT
		SPEED_FLOAT_DEVICE = FADE_FILL_WIDTH
		SPEED_LIMIT_LEVEL_1 = BANKING_MIN_SPEED
		SPEED_LIMIT_LEVEL_2 = 1120
		BANK_BRAKE_SPEED_LIMIT = 1100
		RUN_BRAKE_SPEED_LIMIT = 480
		lifeDrawerX = WALK_COLLISION_CHECK_OFFSET_Y
		passStageActionID = WALK_COLLISION_CHECK_OFFSET_Y
		isNeedPlayWaterSE = NEED_RESET_DEDREE
		MOON_STAR_ORI_X_1 = SCREEN_WIDTH
		MOON_STAR_DES_X_1 = SCREEN_WIDTH - ANI_BAR_ROLL_1
		DEGREE_DIVIDE = New Int[]{ANI_HURT_PRE, 75, 105, Def.TOUCH_HELP_HEIGHT, SpecialMap.MAP_HEIGHT, 255, 285, 316, MDPhone.SCREEN_WIDTH}
		iArr = New Int[YELLOW_NUM]
		iArr[TERMINAL_NO_MOVE] = MAX_ITEM
		iArr[TERMINAL_RUN_TO_RIGHT_2] = TERMINAL_SUPER_SONIC
		iArr[TERMINAL_SUPER_SONIC] = ITEM_RING_5
		TRANS = iArr
		FOCUS_MAX_OFFSET = (MapManager.CAMERA_HEIGHT Shr TERMINAL_NO_MOVE) - BREATHE_IMAGE_WIDTH
		FOOT_OFFSET_X = New Int[]{LEFT_FOOT_OFFSET_X, SIDE_FOOT_FROM_CENTER}
		...
		ATTACK_POP_POWER = GRAVITY + 774
		RANDOM_RING_NUM = New Int[]{TERMINAL_NO_MOVE, MAX_ITEM, MAX_ITEM, MAX_ITEM, MAX_ITEM, TERMINAL_COUNT, SPIN_KEY_COUNT, ANI_YELL, FADE_FILL_WIDTH}
		itemVec = (Int[][]) Array.newInstance(Integer.TYPE, New Int[]{MAX_ITEM, TERMINAL_RUN_TO_RIGHT_2})
		lifeNum = TERMINAL_RUN_TO_RIGHT_2
		timeStopped = NEED_RESET_DEDREE
		uiOffsetX = WALK_COLLISION_CHECK_OFFSET_Y
		NumberSideX = 81 - MyAPI.zoomIn(WALK_COLLISION_CHECK_OFFSET_Y)
		NUM_PIC_WIDTH = ITEM_RING_10
		NUM_PIC_HEIGHT = ANI_POAL_PULL
		NUM_SPACE_ANIMATION = New Int[]{ANI_PUSH_WALL, ANI_PUSH_WALL, BREATHE_IMAGE_WIDTH, ANI_PUSH_WALL, ANI_PUSH_WALL}
		iArr = New Int[MAX_ITEM]
		iArr[WALK_COLLISION_CHECK_OFFSET_Y] = FONT_WIDTH_NUM
		iArr[TERMINAL_NO_MOVE] = FONT_WIDTH_NUM
		iArr[TERMINAL_RUN_TO_RIGHT_2] = FONT_WIDTH_NUM
		iArr[TERMINAL_SUPER_SONIC] = FONT_WIDTH_NUM
		iArr[YELLOW_NUM] = FONT_WIDTH_NUM
		NUM_SPACE_FONT = iArr
		iArr = New Int[MAX_ITEM]
		iArr[WALK_COLLISION_CHECK_OFFSET_Y] = NUM_PIC_WIDTH + TERMINAL_NO_MOVE
		iArr[TERMINAL_NO_MOVE] = NUM_PIC_WIDTH + TERMINAL_NO_MOVE
		iArr[TERMINAL_RUN_TO_RIGHT_2] = NUM_PIC_WIDTH + TERMINAL_NO_MOVE
		iArr[TERMINAL_SUPER_SONIC] = NUM_PIC_WIDTH + TERMINAL_NO_MOVE
		iArr[YELLOW_NUM] = NUM_PIC_WIDTH + TERMINAL_NO_MOVE
		NUM_SPACE_IMAGE = iArr
		NUM_SPACE = NUM_SPACE_ANIMATION
		NUM_ANI_ID = New Int[]{ANI_SMALL_ZERO, ANI_SMALL_ZERO_Y, ANI_BIG_ZERO, StringIndex.FONT_COLON_BIG, ANI_SMALL_ZERO_Y}
		preTimeCount = WALK_COLLISION_CHECK_OFFSET_Y
		BG_NUM = ((SCREEN_WIDTH + BACKGROUND_WIDTH) - TERMINAL_NO_MOVE) / BACKGROUND_WIDTH
		isbarOut = NEED_RESET_DEDREE
		movespeedx = 96
		movespeedy = ANI_PULL_BAR_MOVE
		clipspeed = MAX_ITEM
		score1 = 49700
		score2 = 12700
		STAGE_PASS_STR_SPACE_FONT = MyAPI.zoomIn(MFGraphics.stringWidth(ANI_POP_JUMP_UP, "\u7d22\u5c3c\u514b\u5b8c\u6210\u884c\u52a81")) + SPIN_KEY_COUNT
		STAGE_PASS_STR_SPACE = 182
		NUM_DISTANCE = NUM_SPACE[WALK_COLLISION_CHECK_OFFSET_Y] * ANI_PUSH_WALL > 60 ? NUM_SPACE[WALK_COLLISION_CHECK_OFFSET_Y] * ITEM_RING_10 : 60
		isStartStageEndFlag = NEED_RESET_DEDREE
		isOnlyBarOut = NEED_RESET_DEDREE
		onlyBarOutCnt = WALK_COLLISION_CHECK_OFFSET_Y
		onlyBarOutCntMax = BACKGROUND_WIDTH
		cursorMax = MAX_ITEM
		PAUSE_FRAME_WIDTH = FONT_WIDTH * ITEM_RING_10
		PAUSE_FRAME_WIDTH = PAUSE_FRAME_WIDTH
		PAUSE_FRAME_WIDTH += YELLOW_NUM
		PAUSE_FRAME_HEIGHT = (MENU_SPACE * MAX_ITEM) + SPIN_KEY_COUNT
		PAUSE_FRAME_OFFSET_X = (-PAUSE_FRAME_WIDTH) Shr TERMINAL_NO_MOVE
		PAUSE_FRAME_OFFSET_Y = (-PAUSE_FRAME_HEIGHT) Shr TERMINAL_NO_MOVE
		PAUSE_MENU_NORMAL_NOSHOP = New Int[]{SPIN_LV2_COUNT, 81, MAX_ITEM, ITEM_RING_5}
		PAUSE_MENU_NORMAL_SHOP = New Int[]{SPIN_LV2_COUNT, 81, ANI_CAUGHT, MAX_ITEM, ITEM_RING_5}
		PAUSE_MENU_RACE_ITEM = New Int[]{SPIN_LV2_COUNT, 82, ANI_SLIP, 81, MAX_ITEM, ITEM_RING_5}
		fadeAlpha = FADE_FILL_WIDTH
